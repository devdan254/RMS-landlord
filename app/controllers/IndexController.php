...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$rec = get_active_user('id');
$table_data = array(
    "log_in_out" => 3
);
$db->where("id",$rec);
$bool = $db->update("login", $table_data);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("user_name","email","password","photo","phone","national_id","role"); //registration fields
			...
			$this->rules_array = array(
				...
				'password' => 'required',
				'photo' => 'required',
				'phone' => 'required|max_len,13|min_len,12',
