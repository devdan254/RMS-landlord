...
class LoginController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				login.role LIKE ? OR 
				login.fb_link LIKE ? OR 
				login.twit_link LIKE ? OR 
				login.insta_link LIKE ? OR 
				login.domain LIKE ? OR 
				login.log_in_out LIKE ?
			)";
			$search_params = array(
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("user_name","email","password","photo","phone","national_id","role","fb_link","twit_link","insta_link","domain");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","user_name","photo","phone","fb_link","twit_link","insta_link","domain");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->validate_captcha = true; //will check for captcha validation
			$this->rules_array = array(
				...
				'fb_link' => 'valid_url',
				'twit_link' => 'valid_url',
				'insta_link' => 'valid_url',
			);
			$this->sanitize_array = array(
				'user_name' => 'sanitize_string',
				'photo' => 'sanitize_string',
				'phone' => 'sanitize_string',
				'fb_link' => 'sanitize_string',
				'twit_link' => 'sanitize_string',
				'insta_link' => 'sanitize_string',
				'domain' => 'sanitize_string',
			);
